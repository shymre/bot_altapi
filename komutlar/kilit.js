...
exports.run = (client, msg, args) => {
    ...
    if (!time) return msg.reply('Doðru kullaným: ' + prefix + 'kilit <süre örneðin: 2 min>');

    if (validUnlocks.includes(time)) {
        msg.channel.overwritePermissions(msg.guild.id, {
            SEND_msgS: null
        }).then(() => {
            msg.channel.sendmsg('Kanal kilidi açýldý.');
        ...
        msg.channel.overwritePermissions(msg.guild.id, {
            ...
            msg.channel.sendmsg(`Kanal kilitlendi. ${ms(ms(time), { long: true })}`).then(() => {
                ...
                client.lockit[msg.channel.id] = setTimeout(() => {
                    msg.channel.overwritePermissions(msg.guild.id, {
                        SEND_msgS: null
                    }).then(msg.channel.sendmsg('Kanalýn kilidi açýldý.')).catch(console.error);
                    delete client.lockit[msg.channel.id];
                }, ms(time));
